...
int main(int argc,char *argv[]){
    ...
    char *dest_ip = argv[1]; //holds the destenation ip
    ...
    printf("PING %s (%s) : %d data bytes \n",dest_ip,inet_ntoa(dest_in.sin_addr),bytes_sent);
    }
    ...
    while ((bytes_received = recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr *)&dest_in, &len)))
    {
        if (bytes_received > 0)
        {
            ...
            printf("%ld bytes from %s to %s : icmp_seq=%d ttl=%d time=%f ms\n", bytes_received,inet_ntoa(dest_in.sin_addr),
            SOURCE_IP,icmphdr->un.echo.sequence,iphdr->ttl,milliseconds);
            break;
        }
    }
    cnt++;
